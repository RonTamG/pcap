...
#![cfg_attr(feature = "runtime-loading", allow(non_upper_case_globals))]

#[cfg(feature = "runtime-loading")]
use std::env;
#[cfg(feature = "runtime-loading")]
use once_cell::sync::Lazy;
#[cfg(all(windows, feature = "runtime-loading"))]
use libloading::os::windows::Library;
#[cfg(all(not(windows), feature = "runtime-loading"))]
...
#[cfg(feature = "runtime-loading")]
crate::library_static!(LIBRARY, "wpcap.dll", |_| {
    env::set_var("PATH", format!(r"C:\Windows\System32\Npcap;{}", env::var("PATH").unwrap()),
    );
    Library::new("wpcap.dll")
});

#[cfg_attr(all(test, not(feature = "runtime-loading")), automock)]
pub mod ffi {
    use crate::library_load;
...
#[cfg(all(test, not(feature = "runtime-loading")))]
pub use mock_ffi::*;
